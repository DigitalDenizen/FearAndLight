...
onready var itemDrop_scene = preload("res://Characters/ItemDrops/WendigoHead.tscn")
...
func _physics_process(delta):
	...
	if alive && not_attacking:
		...
		if targetObject.path.size() > 2:
			...
			_on_Wendigo_melee(Melee, targetObject.targetObject.global_position, global_position)
	else:
		if not_attacking:
...
func _on_Wendigo_melee(Melee, target_pos, wendigo_pos):
	...
	not_attacking = false
	attackCountDown = 50
	var slash = Melee.instance()
	slash.attacker = "Wendigo"
	add_child(slash)
	slash.shoot(target_pos, wendigo_pos)
